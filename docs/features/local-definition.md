...
記事内のみ有効なローカル定義。`:::definition{#id title="タイトル"}` で定義し、`[[#id]]` または `[[term]]` で参照する。
...
```html
<div class="definition-block" id="local-f" data-def-title="写像 f">
  <p>$f : A \to B$ を...と定義する。</p>
</div>
```
...
- `id` 属性でページ内アンカーになる
- `title` 属性を `data-def-title` として出力し、表示ラベルは `▶ 定義 (写像 f)` 形式にする
- `title` が省略された場合は既存コンテンツ互換のため `id` を表示タイトルとして使う
...
| ケース | 期待出力 |
|--------|---------|
| 基本: `:::definition{#id title="..."}` | `<div class="definition-block" id="id" data-def-title="...">` |
...
| `title` 省略 | `data-def-title` に id を入れて表示タイトルにする |

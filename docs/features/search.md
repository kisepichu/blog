...
- `package.json` の `build` スクリプトを更新:
  ...
  ```
  `sh -c '...' --` 形式にすることで、`pnpm build -- --site X --base Y` のように渡された引数 (`$@`) が `astro build` に中継され、`pagefind` には届かない。
- GitHub Actions (`.github/workflows/deploy.yml`): `pnpm build -- --site ... --base ...` で実行しているため、上記形式が必要。workflow 自体の変更は不要。
- 生成物: `dist/pagefind/` — Pagefind JS ランタイム + インデックス (`.gitignore` 対象)

### ヘッダー検索ボックス

`Layout.astro` のヘッダーナビ内、ナビリンクの右に `HeaderSearch` React island を配置する。

```astro
<HeaderSearch client:load baseUrl={import.meta.env.BASE_URL} />
```

`src/components/HeaderSearch.tsx` — React island。

- `#<tag>` / `@<type>` 入力で Pagefind `filters()` を使ったタグ補完ドロップダウンを表示
- 候補クリック / Enter 選択 (ドロップダウンあり) → 入力中の `#xxx` / `@xxx` 部分を選択値で置換して input に反映 (遷移しない)
- Enter (ドロップダウンなし) → `/search?q=<query>` に `window.location.href` 遷移
- Escape → ドロップダウンを閉じる (`type="search"` のブラウザデフォルト動作を `preventDefault()` で抑制)
- マウント時に `window.location.search` の `?q=` を input に反映
- スタイル: `HeaderSearch.module.css` (CSS Modules)
...
| `#` | 全タグ候補をドロップダウン表示 |
| `#<部分文字列>` | 部分一致するタグ候補を表示 |
| `@` | type 候補 (`post`, `definition`) を表示 |
| `poset #` | `poset` をプレフィックスとして保持しつつ tag 候補を表示し、選択で `poset #tag` を input に挿入 |
| 上記以外 | Enter で `/search?q=<query>` に遷移 |

#### ARIA

- `role="combobox"`, `aria-expanded`, `aria-autocomplete="list"`
- ドロップダウン: `role="listbox"` + `role="option"` + `aria-selected`
- `aria-controls` はドロップダウン表示時のみ設定 (非表示時は `undefined`)
...
- `src/components/Layout.astro` — ヘッダー検索ボックス (HeaderSearch island) 追加、`activePage` 型拡張
- `src/components/HeaderSearch.tsx` — ヘッダー検索 React island (タグ補完・遷移)
- `src/components/HeaderSearch.module.css` — HeaderSearch のスタイル

...
| `.definition-block::before` | `content: "▶ 定義"` label (font-ui, 0.68rem, accent-dim) |
...
| definition-block | teal 枠・accent-bg 背景・`▶ 定義` ラベルが表示される |
...
|---|---|
| `aliases` が空 | 別名表示なし |
| `aliases` が `id` のみ | 「別名:」行を非表示 (id は別途 id-badge で表示済み) |

...
container (max-width: 900px, padding: 2.5rem 1.25rem)
  .home-hero
    .home-hero__prompt   "~/blog $ ls"
    h1.home-title        "きせのブログ"
    p.home-subtitle      "型理論 · 論理学 · 数学 · プログラミング言語理論"
  ...
  .home-grid  (grid-template-columns: 1fr 280px; gap: 3rem)
    ...
    ── aside (280px) ──
      ...
      .section-title "最近の定義"  ← defs がある場合のみセクションごと表示
        (margin-top: 2rem)
      .def-compact
        <a class="def-compact-item" href="${base}/defs/[id]"> × 最大4件
...
Phase 2 では **id のアルファベット順** で先頭4件（暫定）。
Phase 3 以降で「最近追加された定義」（git 更新日時ベース等）への変更を検討。
...
| 最近の定義 | `.def-compact-item` が最大4件表示、`/defs/[id]` へのリンク |
| 定義なし | 最近の定義セクションが非表示 |
...
- 最近の定義の並び順: Phase 2 は id アルファベット順 (暫定)。Phase 3 以降に最終決定。

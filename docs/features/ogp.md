...
definition_block の先頭 120 文字 (HTML タグを除去したプレーンテキスト) を使う。
...
- Markdown→プレーンテキスト変換では見出し・コードブロック・数式を除去し、通常段落のみ使う。
...
| ケース | 挙動 |
|---|---|
| `description` なし + 本文が空 | 空文字でも許容。description タグを空文字で出力する |

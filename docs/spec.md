...
Astro integration の `astro:config:setup` フックで構築し、`remarkPlugins` と backlink ビルダーの両方に渡す:
...
本番ビルド時は draft/scrap の Definition を alias map から除外する。

### remark/rehype プラグイン (per-file)

1. **remark-definition-block**: `:::definition` → definition_block ノード (remark-directive を利用)
2. **remark-concept-link**: alias map と `baseUrl` を受け取り、`[[term]]` を変換する
...
- **backlink グラフ**: alias map を使って `[[term]]` を canonical id に正規化しながら全ファイルをスキャンし、各ページの backlink 一覧を生成してページ props に渡す (concept-link と同じ resolver を共有)
